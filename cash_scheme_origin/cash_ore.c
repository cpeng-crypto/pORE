...
int ore_compare(int *result_p, ore_ciphertext ctxt1, ore_ciphertext ctxt2, pairing_t pairing) {
  ...
  for (i = 0; i < nbits; i++) {
    for (j = 0; j < nbits; j++) {
      ...
      pairing_apply(temp3, ctxt2->cipher[i]->cipher_b, ctxt1->cipher[j]->cipher_c, pairing);
      pairing_apply(temp4, ctxt2->cipher[i]->cipher_a, ctxt1->cipher[j]->cipher_d, pairing);
