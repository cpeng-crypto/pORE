...
int ore_compare(int *result_p, ore_ciphertext ctxt1, ore_ciphertext ctxt2, pairing_t pairing) {
  ...
  element_t temp1[nbits], temp2[nbits], temp3[nbits], temp4[nbits];
  for(i = 0; i < nbits; i++)
  {
    ...
    pairing_apply(temp1[i], ctxt1->cipher[i]->cipher_a, ctxt2->cipher[i]->cipher_d, pairing);
    pairing_apply(temp2[i], ctxt1->cipher[i]->cipher_b, ctxt2->cipher[i]->cipher_c, pairing);
    pairing_apply(temp3[i], ctxt2->cipher[i]->cipher_b, ctxt1->cipher[i]->cipher_c, pairing);
    pairing_apply(temp4[i], ctxt2->cipher[i]->cipher_a, ctxt1->cipher[i]->cipher_d, pairing);
  }
  ...
  for (i = 0; i < nbits; i++) {
    for (j = 0; j < nbits; j++) {
      ...
      if (!element_cmp(temp3[i], temp4[i])) {
        res = -1;
        break_flag = true;
        break;
      }
  ...
  *result_p = res;
  for(i = 0; i < nbits; i++)
  {
    element_clear(temp1[i]);
    element_clear(temp2[i]);
    element_clear(temp3[i]);
    element_clear(temp4[i]);
  }
  return ERROR_NONE;
}
